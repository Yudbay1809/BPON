        ...
        <div className="container mx-auto px-4 md:px-8 max-w-7xl text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            {copy.certifications.map((certification) => (
              <span key={certification} className="bg-white/10 border border-white/30 text-white text-sm font-semibold px-4 py-2 rounded-full">
                {certification}
              </span>
            ))}
          </div>
            ...
            {copy.catalog.items.map((product, index) => {
              ...
              return (
                <div key={product.id} className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                  <div className={`${index % 2 !== 0 ? 'lg:order-last' : ''} relative h-[360px] rounded-2xl overflow-hidden shadow-xl`}>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {product.applications.map((application) => (
                          <span key={application} className="text-xs bg-primary/8 text-primary px-3 py-1.5 rounded-full border border-primary/20 font-semibold">
                            {application}
                          </span>
                        ))}
                      </div>
